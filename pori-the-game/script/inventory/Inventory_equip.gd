...
var scene_path: String = "res://scenes/décor_explo/Inventory_equip.tscn"
...
func pickup_item():
	...
	if Inventory.player_node:
		Inventory.add_item(item, Inventory.inventory_equipement)
		self.queue_free()
...
func _on_area_2d_body_entered(body: Node2D) -> void:
	if body.is_in_group("Player"):
		...
		body.color_rect.global_position = body.global_position + Vector2(-145, -100)
		body.color_rect.visible = true

# Si le joueur n'est pas dans la zone de collision de l'item, alors il ne peut pas le prendre
func _on_area_2d_body_exited(body: Node2D) -> void:
	if body.is_in_group("Player"):
		player_in_range = false
		body.color_rect.visible = false
